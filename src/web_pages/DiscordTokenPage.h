const char DiscordTokenPageHtmlContent[] PROGMEM = R"rawliteral(<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    ...
    <style>
      * {
      ...
      #form-button-submit:hover {
        background-color: #4851e6;
      }

      #form-button-submit:active {
        background-color: #3034a9;
      }
  ...
  <body>
    <h1 id="header-text">Discord Chat</h1>

    <form id="form-container">
      <h4 id="form-info-text">Paste Your Bot Key Here</h4>
      <input id="form-bot-key-input" type="text" />
      <button id="form-button-submit">Submit</button>
      <div id="success-message" style="display: none">
        Bot key saved successfully!
      </div>
      <div id="error-message" style="display: none">Failed to save bot key</div>
    </form>
  </body>
  <script>
    fetch("/form-bot-key-get", {
      method: "GET",
    })
      .then((response) => response.json())
      .then((data) => {
        document.getElementById("form-bot-key-input").value = data.bot_key;
      })
    ...
    document
      ...
      .addEventListener("submit", function (event) {
        ...
        const botKey = document.getElementById("form-bot-key-input").value;

        fetch("/form-bot-key-set", {
          method: "POST",
          headers: {
